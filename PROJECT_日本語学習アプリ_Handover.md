...
> **Cập nhật lần cuối:** 2026-09-22 — **PM xác nhận bản APK debug của gói 2026-09-13 (commit `f95d252`/`aace5f8`) đã test ổn trên điện thoại thật.** Theo đúng thỏa thuận ở mục 5.4, đã bump `versionCode 2`/`versionName "0.1"` và build lại APK debug (verify đủ 4 plugin, đúng version, `RECORD_AUDIO`), `main` cũng đã push lên `origin/main` xong (không còn ahead nữa). **Cùng phiên này: thêm module thứ 3 シャドウイング (FR_013)** — xem mục 4.15. Đã code (`build_app.py` đọc thẳng `Shadowing_Data.json` mới, `app_template.html` thêm `shadowDashboard()`/`shadowUnitScreen()`) + build (663 từ + 38 track + 240 đoạn shadowing) + test qua Browser pane xong. **PM tự test bản PC (mở HTML trực tiếp) và góp ý redesign UI qua 2 vòng** (mục 4.16, 4.17): gộp 🎧 nghe + 🎤 luyện nói vào 1 accordion mỗi Đoạn, UI riêng màn Unit chuyển tiếng Nhật (cả dashboard), layout gọn hơn, dời dòng đếm lượt lên hàng tab, thêm 3 màu tiến độ + badge điểm mỗi lượt thoại (đổi hẳn schema lưu điểm sang "điểm lần ghi gần nhất" thay vì điểm cao nhất) — đã sửa + test lại xong cả 2 vòng. **Đã build APK debug đo dung lượng thật: ~125MB** (`versionCode 3`/`versionName "0.1"`, giữ nguyên tên phiên bản vì chưa phải mốc ổn định mới) — **CHƯA cài/test trên điện thoại thật**. Dữ liệu `06_ Shadowing_Jokyu/` **theo yêu cầu PM: không commit** — quyết định về bitrate audio (giữ 64kbps hay hạ 48kbps) để sau khi PM test APK trên điện thoại thật, xem mục 4.17. Trạng thái các phần còn lại giữ nguyên như bản 2026-09-13: đủ Phase 2 (FR_008→FR_012) + FR_006/FR_007 + redesign dashboard + 4 việc mục 4.14 (checkbox "Học từ đã nhớ", lưới Bộ 6 icon, hero streak, Cài đặt sticky header). Repo git vẫn chỉ còn **`main` + 1 worktree** (`handover-file-review-9bd2f7` — PM chưa xóa, phiên 2026-09-22 chạy tiếp trong đúng worktree này nhưng trên 1 branch khác `claude/read-handover-file-b582d2`, xem mục 4.15) — xem mục 6. Trước đó nữa, dự án đã trải qua "Phase 1" (2026-09-11): viết lại tầng đọc `build_app.py` (v2, hỗ trợ cả 3 kiểu ô OOXML) sau khi từ điển đổi cách lưu làm build chết, thêm sổ khóa `_id_lock.json`. Đọc kỹ mục 4.9 và `CLAUDE.md` quy tắc #6 nếu thấy nhắc `_id_lock.json`.
...
- Remote: `origin` → https://github.com/Nin-Min2231/Nihongo_App.git (đổi tên 2026-09-12, xem mục 4.12 — URL cũ `Lading_page-VS` vẫn redirect được), nhánh `main`. **Đã push xong** tới `aace5f8` (xác nhận lại 2026-09-22, mục 4.15 — `git log origin/main..main` rỗng), không còn ahead. Riêng bump version `versionCode 2`/`"0.1"` (mục 5.4/4.15) và toàn bộ code FR_013 (mục 4.15) **CHƯA commit** — hỏi PM trước khi commit theo đúng quy ước dưới đây.
- `.gitignore` loại trừ: `node_modules/`, audio trùng lặp (chỉ giữ `02_IT_Gyoumuhen/AudioCD/` làm nguồn duy nhất, bỏ 2 bản copy ở `01_Build_App/audio/` và `03_Android_App/www/` — `01_Build_App/audio/shadowing/` của module mới cũng nằm trong diện loại trừ này vì là con của `01_Build_App/audio/`), `02_IT_Gyoumuhen/reading_segments/audio/` (~37MB, giữ `segments_data.json`), `05_json/` (bản xuất tiến độ cá nhân), APK, Gradle/Android build artifacts, `local.properties` (machine-specific), `.claude/settings.local.json`. **`06_ Shadowing_Jokyu/` (nguồn: JSON + xlsx + 46MB audio) KHÔNG nằm trong `.gitignore`** — nếu commit thì sẽ là 46MB dữ liệu mới vào repo, giống cách `02_IT_Gyoumuhen/AudioCD/` đã được commit làm nguồn chính thức; nội dung có bản quyền giáo trình nên chỉ hợp lý khi repo là riêng tư/nội bộ (xem `CLAUDE.md` mục FR_013).
